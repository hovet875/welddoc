...
export type OpenTraceabilityEditModalOpts = {
  modalMount: HTMLElement;
  signal: AbortSignal;
  isAdmin: boolean;
  projectId: string;
  types: TraceabilityTypeRow[];
  optionsDn: TraceabilityOptionRow[];
...
const renderDynamicFields = (
  type: TraceabilityTypeRow,
  values: Partial<ProjectTraceabilityRow> = {},
  opts: {
    optionsDn: TraceabilityOptionRow[];
    optionsSch: TraceabilityOptionRow[];
    optionsPn: TraceabilityOptionRow[];
  ...
  const { optionsDn, optionsSch, optionsPn, optionsFiller, materials } = opts;
  const dn = values.dn ?? "";
...
export function openTraceabilityEditModal(opts: OpenTraceabilityEditModalOpts) {
  const {
    modalMount,
    signal,
    isAdmin,
    ...
    `
      <div class="modalgrid">
        ...
        <div class="trace-dynamic" data-dynamic-fields>
          ${renderDynamicFields(initialType, row ?? {}, { optionsDn, optionsSch, optionsPn, optionsFiller, materials })}
        </div>
